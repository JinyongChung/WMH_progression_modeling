...
p_perm_k_norm = p_perm_k_weighted./repmat(sum(sum(p_perm_k_weighted+eps(0),2),3),[1 N+1 N_S]);
...
for s = order_seq
    ...
    for i = order_bio
        ...
        for index = 1:length(possible_positions)
            ...
            possible_likelihood(index) = sum(log(total_prob_subj+eps(0)));
        end
...
likelihood_opt = sum(log(total_prob_subj+eps(0)));
