...
glme = fitglme(tb_control, 'WMHv ~ 1 + AGE','Link','log','Distribution','Poisson','FitMethod','Laplace'); % ROI-wise poisson mixed-effects regression model
resid_control = residuals(glme); % residuals: AGE-adjusted WMH volume
WMHseverity_ROIwise_stroke(sidx_w_stroke,ROInum) = (WMHvolume_ROIwise_stroke(sidx_w_stroke,ROInum)-mean(resid_control))./std(resid_control);
WMHseverity_ROIwise_control(sidx_w_control,ROInum) = (WMHvolume_ROIwise_control(sidx_w_control,ROInum)-mean(resid_control))./std(resid_control);
end
